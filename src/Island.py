...
class Island:
    def __init__(self, pin, evaluator, diversity, parameters, selection, migration, replacement, reproduction,
                 population_size, tmp_dir):
        # Island
    ...
    def sort_individuals(self, how):  # from the fittest to the least fit
    ...
    def print_generation_summary(self):
        print('island [', self.pin, '], fittest [', '{0:.2f}'.format(self.individuals[0].fitness), '], entropy [',
              '{0:.2f}'.format(self.diversity_measure.entropy), '], mean [', '{0:.2f}'.format(self.average_fitness),
