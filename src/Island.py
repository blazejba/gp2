...
class Island:
	def __init__(self, name, configs, chromosome_length, all_evaluators, tmp_dir):
		...
		self.replacement_policy = ReplacementPolicy(replacement_config, migration_config, tmp_dir, name, self.population_size)
		self.reproduction_policy = ReproductionPolicy(self.population_size, reproduction_config, chromosome_length,
		                                              self.terminal_set, self.operator_set)
	...
	def sort_individuals(self):
		...
		print('is', self.island_name, 'the fittest', self.individuals[0][0], 'in gen', self.generation)
	...
	def next_generation(self):
		self.replacement_policy.migration_policy.migrate_out(self.individuals)
		self.evolve()
		self.start_evaluations()

	def start_evaluations(self):
		for index, individual in enumerate(self.individuals):
			if individual[2]:  # don't evaluate elites/previously evaluated
				continue
			genome = ''.join(str(gene) for gene in individual[1])
			terminal_command = ["python3", self.evaluation_function_path, genome, str(index)]
			self.processes.append(subprocess.Popen(terminal_command, stdout=subprocess.PIPE))
